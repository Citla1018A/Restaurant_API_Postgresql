...
exports.create = (req, res) => {
    ...
    if (!req.body.nombre) {
        res.status(400).send({
            message: "El contenido no puede ser vacio, nombre=" + req.body.nombre + " descripcion=" + req.body.descripcion
        });
        return;
    }
...
exports.findAllEstatus = (req, res) => {
    Rol.findAll({ where: { estatus: true } })
        ...
        .catch(err => {
            res.status(500).send({
                message:
                    err.message || "Ocurrio un error al recuperar Roles por estatus activo."
            });
        });
};